...
class GradientEditing(nn.Module):
    def __init__(self, nu: Tensor) -> None:
        super().__init__()
        


class TerminalDispersionBound(Bound):
    def __init__(self, clip=None, flatness=True) -> None:
        super().__init__()
        self.n_iter = 0
        self.clip = clip
    ...
    @property
    def name(self):
        return "terminal_dispersion" if not self.flatness else "termnial_dispersion+flatness"
    ...
    def trajectory_term(self, parallel_model: ParallelModel, delta=None, *args, **kwargs):
        term_disp = parallel_model.terminal_dispersion(delta)
        if self.n_iter == 0:
            if term_disp == 0:
                return 0
            else:
    ...
    def surrogate_forget(self, C: Tensor, nu: 'list[list[Tensor]]', parallel_model: ParallelModel, parallel_training_data_loader: ParallelDataloader, test_dataloader: DataLoader):
        ...
        diff_grad = [[
            # grad_population[index_model][index_param] 
                - grad_empirical[index_model][index_param] for index_param in range(len(grad_empirical[0]))] for index_model in range(len(grad_empirical))] 
        Delta = iHVP(
            parallel_model,
            [[
            ...
            [[
                2 * C * nu[index_model][index_param] - diff_grad[index_model][index_param]  
